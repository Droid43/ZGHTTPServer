...
@interface ZGHTTPRequestHandler ()
...
@implementation ZGHTTPRequestHandler
...
- (instancetype)initWithHeadData:(NSData *)data
    ...
    if(self = [self init]){
        ...
        _bodyDataOffset = 0;
        NSString *length = [[_requestHead headDic] objectForKey:@"Content-Length"];
...
- (NSError *)invalidError{
    if(![_requestHead.protocol isEqualToString:@"HTTP"]) return [NSError errorWithDomain:[NSString stringWithFormat:@"服务器不支持%@协议❌",_requestHead.protocol]
                                                                                    code:501
                                                                                userInfo:nil];
    if(![_requestHead.version isEqualToString:@"1.1"]) return [NSError errorWithDomain:[NSString stringWithFormat:@"服务器不支持%@协议版本❌",_requestHead.version]
                                                                                    code:501
                                                                                userInfo:nil];
    NSError *error = [self refuseError];
    if(error) return error;
    if([_requestHead.method isEqualToString:@"POST"] || [_requestHead.method isEqualToString:@"POST"]){
        if(_bodyDataLength == 0) return [NSError errorWithDomain:[NSString stringWithFormat:@"请求参数出错，%@方法需要指定body长度❌",_requestHead.method]
                                                            code:411
                                                        userInfo:nil];
...
- (BOOL)loadData:(NSData *)data{
    ...
    [headArray enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        [head setObject:[headItems[1] stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding] forKey:headItems[0]];
    }];
